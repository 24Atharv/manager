export const MiddleSlider = () => {
    return <div>
        <span className="text-black flex flex-col gap-5 p-6 w-102 bg-gray-300 rounded-lg">
            <span className="flex gap-3 items-center">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" className="size-10 text-green-500">
                <path stroke-linecap="round" stroke-linejoin="round" d="M16.5 3.75V16.5L12 14.25 7.5 16.5V3.75m9 0H18A2.25 2.25 0 0 1 20.25 6v12A2.25 2.25 0 0 1 18 20.25H6A2.25 2.25 0 0 1 3.75 18V6A2.25 2.25 0 0 1 6 3.75h1.5m9 0h-9" />
            </svg>
            <span className="text-2xl">Link Management</span>
            </span>
            <span>Never lose important bookmarks with smart categorization, easy retrieval, and tagging.</span>
        </span>
    </div>
}

